...
export class ComputePipeline extends Pipeline {
    ...
    bufferIOs;
    textureIOs;
    checkedBufferIO = false;
    checkedTextureIO = false;
    constructor() {
        super();
    ...
    initFromObject(descriptor) {
        ...
        const t = [];
        for (let z in descriptor.computeShader.inputs)
            t.push({ name: z, ...descriptor.computeShader.inputs[z] });
        //console.log("TTTTTT ", t)
        this.computeShader.inputs = t; //descriptor.computeShader.inputs;
        if (descriptor.computeShader.code)
            this.computeShader.code.text = descriptor.computeShader.code;
        ...
        if (this.bufferIOs) {
            ...
            if (this.onReceiveData) {
                ...
                if (!this.stagingBuffer)
                    this.stagingBuffer = SLGPU.createStagingBuffer(this.bufferSize);
